...
BEGIN
  ...
  IF EXISTS (
    ...
    WHERE id = p_quote_id 
    OR parent_quote_id = p_quote_id
  ) THEN
    RAISE EXCEPTION 'Failed to delete quote or its additives';
  END IF;

  -- Verify no orphaned data exists
  IF EXISTS (
    SELECT 1 FROM quote_history WHERE quote_id = p_quote_id
    UNION ALL
    SELECT 1 FROM quote_notes WHERE quote_id = p_quote_id
    UNION ALL
    SELECT 1 FROM quote_budgets WHERE quote_id = p_quote_id
    UNION ALL
    SELECT 1 FROM quote_work_budgets WHERE quote_id = p_quote_id
    UNION ALL
    SELECT 1 FROM quote_settings WHERE quote_id = p_quote_id
    UNION ALL
    SELECT 1 FROM distribution_categories WHERE quote_id = p_quote_id
  ) THEN
    RAISE EXCEPTION 'Orphaned data still exists after quote deletion';
  END IF;
END;
$$;
